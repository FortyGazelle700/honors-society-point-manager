...
export default function EventsClientPage({
  ...
  useEffect(() => {
    ...
    if (
      ...
      events.at(-1)!.name != emptyEvent().name
    ) {
    ...
    <form action={action} className="flex flex-col gap-4 pb-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <SaveButton
          disabled={(() => {
            return events.some(
              (e) => events.filter((f) => e.name == f.name).length > 1,
            );
          })()}
        />
      </div>
      <div className="flex flex-col gap-6 md:gap-2">
        ...
        {events.map((event) => {
          ...
          qrURL.searchParams.set("eventName", event.name ?? "");
          qrURL.searchParams.set("eventId", event.id ?? "");
          ...
          return (
            <div className="flex gap-2" key={String(event.id)} id={event.id}>
              <div className="flex flex-1 flex-col items-start gap-2 md:flex-row md:items-center">
                <Input
                  ...
                  onChange={(e) => {
                    setEvents((prev) =>
                      prev.map((o) =>
                        o.id == event.id ? { ...o, name: e.target.value } : o,
                      ),
                    );
                  }}
                  onPaste={(e) => handlePaste(e.target as HTMLInputElement)}
                  className="h-9 md:w-[40ch]"
                  placeholder="Music Stand Project"
                />
                <Select
                  value={event.type}
                  onValueChange={(value) => {
              ...
              <div className="flex flex-col items-center gap-2 sm:flex-row">
                      ...
                      <div className="flex items-center justify-end gap-2 pt-4">
                        <Button
                          ...
                          onClick={(evt) => {
                            evt.preventDefault();
                            window.open(
                              `/qr?value=${encodeURIComponent(qrURL.toString())}&code=${encodeURIComponent(event.verificationCode ?? "")}&label=${encodeURIComponent(event.name ?? "")}&print=true`,
                              "qr code",
                        ...
                        <Button
                          ...
                          onClick={(evt) => {
                            evt.preventDefault();
                            window.open(
                              `/qr?value=${encodeURIComponent(qrURL.toString())}&code=${encodeURIComponent(event.verificationCode ?? "")}&label=${encodeURIComponent(event.name ?? "")}`,
                              "qr code",
                              "width=1000,height=500,status=no,toolbar=no",
                            );
                          }}
                ...
                <ResponsivePopover>
                  ...
                  >
                    ...
                    <ResponsivePopoverBody className="flex flex-col gap-2">
                          ...
                          <Popover>
                            <PopoverTrigger asChild>
                              <Button
                                ...
                                className="data-[empty=true]:text-muted-foreground ml-4 h-9 w-[calc(100%-theme(spacing.4))] flex-1 justify-start text-left font-normal"
                              >
                                <CalendarIcon />
                                {event.date ? (
                      ...
                      <label className="flex h-9 w-full items-center justify-between gap-2">
                        <span>Needs User Description</span>
                        <Checkbox
                          className="border-primary/50"
                          value={event.needsAdditionalInfo ? "on" : "off"}
                      ...
                      <label className="flex h-9 w-full items-center justify-between gap-2">
                        <span>Allow QR Code Submission</span>
                        <Checkbox
                          className="border-primary/50"
                          value={event.hasQrSubmission ? "on" : "off"}
                          onCheckedChange={(checked) => {
                            setEvents((prev) =>
                              prev.map((o) =>
                                o.id == event.id
                                  ? { ...o, hasQrSubmission: checked == true }
                                  : o,
                              ),
                            );
                          }}
                ...
                {events.filter((e) => e.name == event.name).length >= 2 && (
                  <Tooltip>
